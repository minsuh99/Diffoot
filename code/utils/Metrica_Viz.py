...
def save_match_clip(hometeam,awayteam, fpath, fname='clip_test', figax=None, frames_per_second=25, team_colors=('r','b'), field_dimen = (106.0,68.0), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False):
    ...
    matplotlib.rcParams['animation.ffmpeg_path'] = r"/usr/bin/ffmpeg"
        ...
        for i in tqdm(index):
            ...
            for team,color in zip( [hometeam.loc[i],awayteam.loc[i]], team_colors) :
                ...
                if include_player_velocities:
                    ...
                    vy_columns = ['{}_vy'.format(c[:-2]) for c in y_columns] # column header for player y positions
                    # 수정 .astype(float)
                    objs = ax.quiver( team[x_columns], team[y_columns], team[vx_columns].astype(float), team[vy_columns].astype(float), color=color, scale_units='inches', scale=10.,width=0.0015,headlength=5,headwidth=3,alpha=PlayerAlpha)
                    figobjs.append(objs)
                if annotate:
                   objs = [ ax.text( team[x]+0.5, team[y]+0.5, x.split('_')[1], fontsize=10, color=color  ) for x,y in zip(x_columns,y_columns) if not ( np.isnan(team[x]) or np.isnan(team[y]) ) ] 
                   figobjs.append(objs)
                # 각주 추가 설정 (수정)
            # plot ball
            ...
            # ball_active이 시간 옆에 표시되도록 설정 (수정)
            ball_active_val = hometeam.loc[i].get('ball_active', 1)
            ball_status = "Active" if ball_active_val else "Inactive"
            timestring = f"{frame_minute}:{frame_second:1.2f} | Ball: {ball_status}"
            ...
            for figobj in figobjs:
                if isinstance(figobj, list):  # 리스트라면 개별적으로 remove 호출
                    for obj in figobj:
                        obj.remove()
                else:
                    figobj.remove()
