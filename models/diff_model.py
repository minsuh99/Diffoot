...
class DiffusionTrajectoryModel(nn.Module):
    def __init__(self, model, num_steps=1000, beta_start=1e-4, beta_end=0.02):
        ...
        # beta schedule
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # ✅ device 명확히 저장
    ...
    def forward(self, x_0, cond_info=None):
        ...
        cond_info = cond_info.permute(0, 3, 2, 1) if cond_info is not None else None
    ...
    def generate(self, shape, cond_info=None, num_samples=10):
        ...
        x_t = x_t.view(num_samples, B, T, 11, 2)  # [N, B, T, 11, 2]
        return x_t
